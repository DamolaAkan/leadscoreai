...
// Records that a solar lead clicked "Claim my free scorecard" on the result
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      console.error("[solar/interest] update error:", error.message);
      return NextResponse.json({ error: "Failed to save" }, { status: 500 });
    }
    if (data?.contact_email) await markDealInterested(data.contact_email);
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[solar/interest] error:", err);
